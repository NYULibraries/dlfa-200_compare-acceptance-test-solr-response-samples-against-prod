...
for f in $( find $ROOT/diffs -type f | sed "s@$ROOT/diffs/@@" | sed 's/.txt//' ); do
    echo -e "<h1>$f</h1>\n" >> $DIFF_REPORT
    ...
    echo "<p/><a href='https://github.com/NYULibraries/dlfa-200_compare-acceptance-test-solr-response-samples-against-prod/blob/develop/diffs/$f.txt'>$f: diff</a>" >> $DIFF_REPORT
    # Temporarily switching to a different repo with the FAB-ified EAD files
   ...
   echo "<p/><a href='https://github.com/NYULibraries/findingaids_eads_test_fabified/commits/master/$f.xml'>$f: EAD file commit history</a>" >> $DIFF_REPORT
